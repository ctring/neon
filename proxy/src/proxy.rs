...
const ERR_CONNECTION_IS_INSECURE: &str = "connection is insecure (try using `sslmode=require`)";
    ...
    loop {
        ...
        match msg {
            ...
            StartupMessage { params, .. } => {
                // `or_else` demands that we use a future here, but it looks nice :)
                let params = async {
                    // Check that the config has been consumed during upgrade
                    // OR we didn't provide it at all (for dev purposes).
                    if tls.is_some() {
                        bail!(ERR_CONNECTION_IS_INSECURE);
                    }
...
enum Error {
    ...
    #[error(transparent)]
    Compute(#[from] compute::ConnectionError),
}
...
impl<S: AsyncRead + AsyncWrite + Unpin> Client<S> {
    ...
    async fn connect_to_db(
        ...
        let connect = async {
            let db_info = self.creds.authenticate(config, &mut self.stream).await?;
            let (db, version, cancel_closure) = db_info.connect().await?;
            let cancel_key_data = session.enable_cancellation(cancel_closure);

            // `Error` type acts as an allowlist for errors.
            Ok::<_, Error>((db, version, cancel_key_data))
        };

        let (db, version, cancel_key_data) = match connect.await {
            Ok(x) => x,
            Err(e) => return self.stream.throw_error(e).await,
        };
        ...
        let mut client = MetricsStream::new(self.stream.into_inner(), inc_proxied);
...
mod tests {
    ...
    async fn handshake_tls_is_enforced_by_proxy() -> anyhow::Result<()> {
        ...
        assert!(client_err.to_string().contains(ERR_CONNECTION_IS_INSECURE));
